...
export default class Documento{
    ...
    @VirtualColumn({query: (alias)=>`CONCAT(${alias}.CSERIEDOCUMENTO, '-', ${alias}.CFOLIO)`
    })
    name:string
    ...
    @Column('decimal',{name:'CNETO', scale:2})
    neto:number
    ...
    @Column('decimal',{name:'CTOTAL', scale:2})
    total:number

    @Column('decimal',{name:'CPENDIENTE', scale:2})
    pendiente:number
    ...
    @OneToMany(()=> Movimiento, mov=>mov.idDocumento)
